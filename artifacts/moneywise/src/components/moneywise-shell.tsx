...
export function BrandMark({ light = false }: { light?: boolean }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className={`display-font text-[1.55rem] font-semibold tracking-[-0.04em] ${light ? 'text-[hsl(var(--sidebar-foreground))]' : 'text-[hsl(var(--foreground))]'}`}>
        moneywise
      </span>
    </div>
  );
...
export function WorkspaceShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="paper-noise min-h-[100dvh] bg-background">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col bg-sidebar px-5 py-6 text-sidebar-foreground transition-transform duration-300 md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-12">
          <p className="px-3 text-[10px] font-bold uppercase tracking-[0.19em] text-sidebar-foreground/45">Your workspace</p>
            ...
            {navItems.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link key={href} href={href} onClick={() => setOpen(false)} className={`focus-ring group flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition-all duration-200 ${active ? 'bg-sidebar-accent text-sidebar-accent-foreground shadow-sm' : 'text-sidebar-foreground/65 hover:bg-sidebar-accent/70 hover:text-sidebar-foreground'}`} data-testid={`link-nav-${label.toLowerCase()}`}>
                  <Icon className={`h-[18px] w-[18px] ${active ? 'text-[hsl(var(--accent))]' : ''}`} />
                  <span>{label}</span>
        ...
        <div className="mt-auto rounded-2xl border border-sidebar-border bg-sidebar-accent/65 p-4">
          ...
          <p className="mt-3 text-sm font-semibold">A little clarity goes a long way.</p>
        ...
        <div className="mt-5 flex items-center gap-3 border-t border-sidebar-border pt-5">
          <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[hsl(var(--accent))] text-xs font-bold text-[hsl(var(--accent-foreground))]" data-testid="avatar-user">{initials(user?.fullName || user?.firstName)}</div>
      ...
      <main className="min-h-[100dvh] md:pl-[248px]">
        <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-border/70 bg-background/85 px-5 backdrop-blur-xl md:px-10">
          ...
          <div className="hidden items-center gap-2 text-xs text-muted-foreground md:flex"><span className="h-2 w-2 rounded-full bg-[hsl(var(--accent))]" /> Your money, in focus</div>
          <div className="ml-auto flex items-center gap-4">
            <Link href="/transactions" className="hidden items-center gap-2 text-sm font-semibold text-primary transition-colors hover:text-[hsl(var(--foreground))] sm:flex" data-testid="link-quick-add"><span className="grid h-6 w-6 place-items-center rounded-md bg-[hsl(var(--accent))] text-lg leading-none">+</span> Add transaction</Link>
            <Link href="/settings" className="group flex items-center gap-2 rounded-xl py-1.5 pl-1.5 pr-2 transition-colors hover:bg-muted" data-testid="link-profile-settings">
              <div className="grid h-8 w-8 place-items-center rounded-full bg-secondary text-xs font-bold text-secondary-foreground">{initials(user?.fullName || user?.firstName)}</div>
